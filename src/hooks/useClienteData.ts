...
export function useClienteData() {
  ...
  const fetchClienteData = useCallback(async () => {
    ...
    try {
      ...
      // Buscar documentos enviados pelo cliente
      ...
      // Buscar lista de documentos obrigatórios (todos os ativos)
      ...
      // Buscar contratos (todos os contratos do cliente, qualquer status)
      ...
      // Processar plano (pegar o contrato mais recente ativo, se houver)
      let planoInfo = null;
      if (contratosData && contratosData.length > 0) {
        const contratoAtivo = (contratosData as any[]).find(c => c.status_contrato === 'ativo') || contratosData[0];
        planoInfo = {
  ...
  return { data, isLoading, error, refetch: fetchClienteData };
}
